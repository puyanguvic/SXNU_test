### 题目 1.1.7
已知一个二维数组 \( A[6][10] \)，每个数组元素占用 4 个存储单元，按照**行优先顺序**存储。若数组元素 \( a[3][5] \) 的存储地址为 1000，求数组元素 \( a[0][0] \) 的存储地址。

选项：
- A. 872  
- B. 860  
- C. 868  
- D. 864  

---

### 解题过程

#### 1. 分析题目条件
- 数组是二维数组，大小为 \( 6 \times 10 \)。
- 每个数组元素占用 4 个存储单元。
- 存储方式为**行优先存储**（即每一行从左到右存储，行与行之间连续）。
- 已知 \( a[3][5] \) 的存储地址为 1000。

行优先存储时，元素 \( a[i][j] \) 的地址公式为：
\[
\text{地址}(a[i][j]) = \text{基地址} + [(i \times \text{列数}) + j] \times \text{元素大小}
\]

其中：
- \( i \) 是行号。
- \( j \) 是列号。
- \( \text{列数} \) 是二维数组的列数（此处为 10）。
- \( \text{元素大小} \) 是每个元素的存储单元数（此处为 4 个存储单元）。

我们要求的是基地址（即 \( a[0][0] \) 的存储地址）。

---

#### 2. 计算已知条件的偏移量
对于 \( a[3][5] \)，行号 \( i = 3 \)，列号 \( j = 5 \)。根据公式：

\[
\text{偏移量} = (i \times \text{列数}) + j
\]

代入数据：
\[
\text{偏移量} = (3 \times 10) + 5 = 30 + 5 = 35
\]

元素 \( a[3][5] \) 相对于基地址的总偏移量（字节数）为：
\[
\text{总偏移字节数} = \text{偏移量} \times \text{元素大小} = 35 \times 4 = 140
\]

---

#### 3. 求基地址
题目中已知 \( a[3][5] \) 的存储地址为 1000，即：
\[
1000 = \text{基地址} + 140
\]

解得：
\[
\text{基地址} = 1000 - 140 = 860
\]

---

### 结果
二维数组的基地址就是 \( a[0][0] \) 的存储地址，因此答案是：
\[
\textbf{B. 860}
\]


### 题目 1.3.4
在实现直接插入排序过程中，假设 \( R[0..i-1] \) 为有序区，\( R[i..n-1] \) 为无序区，现在要将 \( R[i] \) 插入到有序区中，可以通过**二分查找**来确定 \( R[i] \) 在有序区中的可能插入位置。这样做能否改善直接插入排序算法的时间复杂度？为什么？（8 分）

---

### 解答

#### 1. 插入排序的基本过程
插入排序的核心思想是：将无序区的第一个元素插入到有序区的正确位置，保证有序区始终有序。

在标准的插入排序中，需要：
1. 从有序区从后向前依次比较，找到插入位置。
2. 比较次数为 \( O(n) \)（最坏情况下需要遍历整个有序区）。

---

#### 2. 使用二分查找优化的插入排序
通过二分查找，可以在 \( R[0..i-1] \) 中快速找到 \( R[i] \) 的插入位置：
1. **二分查找的时间复杂度**为 \( O(\log i) \)。
2. 找到插入位置后，仍需将插入位置后的元素向后移动，移动的复杂度仍为 \( O(i) \)。

---

#### 3. 优化后的时间复杂度分析
在插入排序中：
- 二分查找减少了查找插入位置所需的比较次数，从 \( O(n) \) 降为 \( O(\log n) \)。
- 但是，插入操作的元素移动次数没有变化，仍需在最坏情况下移动 \( O(n) \) 次。

因此，总体时间复杂度仍然是：
\[
O(n^2)
\]

虽然二分查找降低了比较次数，但移动次数决定了插入排序的时间复杂度，移动操作无法优化，所以时间复杂度并未改变。

---

#### 4. 结论
使用二分查找可以减少插入排序中的**比较次数**，但不会改变其整体时间复杂度，仍为 \( O(n^2) \)，因为元素移动的时间开销占主导地位。

---

### 总结答案
**不能改善插入排序的时间复杂度，原因是元素的移动操作仍需 \( O(n) \) 的时间。**


### 题目

已知一个二维数组 \( A[3][5] \)，每个数组元素占用 2 个存储单元，若按**列优先**顺序进行存储，\( A[0][0] \) 的存储地址为 100，求 \( A[2][3] \) 的存储地址。

选项：
- A. 122  
- B. 126  
- C. 114  
- D. 116  

---

### 解答

#### 1. 分析题目
**列优先顺序**表示存储时先存完同一列，再存下一列。数组中元素的存储地址公式为：
\[
\text{地址}(A[i][j]) = \text{基地址} + [(j \times \text{行数}) + i] \times \text{元素大小}
\]

其中：
- \( i \) 是行号，\( j \) 是列号。
- \( \text{行数} = 3 \)。
- \( \text{元素大小} = 2 \)（每个元素占 2 个存储单元）。
- \( \text{基地址} = 100 \)。

要求 \( A[2][3] \) 的存储地址。

---

#### 2. 确定 \( A[2][3] \) 的偏移量
根据公式，列优先存储时，偏移量为：
\[
\text{偏移量} = (j \times \text{行数}) + i
\]

将 \( i = 2 \)，\( j = 3 \) 代入：
\[
\text{偏移量} = (3 \times 3) + 2 = 9 + 2 = 11
\]

---

#### 3. 计算存储地址
存储地址为：
\[
\text{地址}(A[2][3]) = \text{基地址} + \text{偏移量} \times \text{元素大小}
\]

代入数据：
\[
\text{地址}(A[2][3]) = 100 + 11 \times 2 = 100 + 22 = 122
\]

---

### 答案
\[
\textbf{A. 122}
\]


### 题目

给出如下算法，分析执行 `fun(a, n, 0)` 的时间复杂度，并需要推导过程（7分）。

```cpp
void fun(int a[], int n, int k) {
    if (k == n - 1) {
        for (int i = 0; i < n; i++) {
            printf("%d", a[i]);
        }
    } else {
        for (int i = k; i < n; i++) {
            a[i] = a[i] + i;
        }
        fun(a, n, k + 1);
    }
}
```

---

### 解答

#### 1. 算法流程分析

1. **递归终止条件**  
   当 \( k == n-1 \) 时，算法停止递归，并执行打印操作：
   ```cpp
   for (int i = 0; i < n; i++) {
       printf("%d", a[i]);
   }
   ```
   此部分需要的时间复杂度为 \( O(n) \)。

2. **递归部分**  
   当 \( k \neq n-1 \) 时，算法会执行以下两部分：
   - **循环操作**：
     ```cpp
     for (int i = k; i < n; i++) {
         a[i] = a[i] + i;
     }
     ```
     该循环的执行次数为 \( n - k \)。
   - **递归调用**：
     ```cpp
     fun(a, n, k + 1);
     ```

---

#### 2. 递归的总时间复杂度

**逐层分析**：  
在每一层递归中：
- 第 \( k \) 层的循环次数为 \( n - k \)，时间复杂度为 \( O(n - k) \)。
- 递归的下一层 \( k+1 \) 会继续执行，直到 \( k = n-1 \)。

总时间复杂度可以表示为以下递归公式：
\[
T(n) = (n-1) + (n-2) + \dots + 1 + O(n)
\]

**求解公式**：
这是一个等差数列求和问题：
\[
T(n) = \sum_{k=1}^{n-1} k + O(n) = \frac{(n-1) \cdot n}{2} + O(n)
\]

时间复杂度为：
\[
T(n) = O(n^2)
\]

---

### 3. 结论

函数 `fun(a, n, 0)` 的时间复杂度为：
\[
\textbf{O(n}^2\textbf{)}
\]

### 题目

给定连通图 \( G \) 的边集 \( E = \{ (a, b), (a, e), (a, c), (b, c), (e, d), (d, f), (f, c) \} \)，请从顶点 \( a \) 出发，按照**深度优先遍历（DFS）**规则，得到一种顶点遍历序列。

选项：
- A. abedfc  
- B. acfebd  
- C. aebdfc  
- D. aedfcb  

---

### 解答

#### 1. 深度优先遍历（DFS）的基本原则
深度优先遍历从起始节点 \( a \) 开始，按照如下规则：
1. 遍历一个节点时，尽量先访问与其直接相连且未被访问的节点。
2. 如果当前节点的所有相邻节点都已访问，则回退到上一层，继续访问其未访问的邻居节点。
3. 遍历的结果依赖节点访问的顺序，因此需要明确边集的顺序。

---

#### 2. 根据题目构造图
由边集 \( E = \{ (a, b), (a, e), (a, c), (b, c), (e, d), (d, f), (f, c) \} \)，可以绘制图的邻接关系：
- \( a \) 连接 \( b, e, c \)
- \( b \) 连接 \( c \)
- \( e \) 连接 \( d \)
- \( d \) 连接 \( f \)
- \( f \) 连接 \( c \)

---

#### 3. 按照深度优先遍历规则操作
从顶点 \( a \) 出发，按照深度优先规则进行遍历：
1. 访问 \( a \)，然后选择第一个未访问的邻居节点 \( b \)。
2. 访问 \( b \)，然后选择 \( b \) 的邻居节点 \( c \)。
3. 访问 \( c \)，没有未访问的邻居，回退到 \( b \) 再回退到 \( a \)。
4. 从 \( a \) 的下一个邻居节点 \( e \) 出发。
5. 访问 \( e \)，然后访问其邻居 \( d \)。
6. 访问 \( d \)，然后访问其邻居 \( f \)。
7. 访问 \( f \)，然后访问其邻居 \( c \)，但 \( c \) 已访问，无需重复访问。

因此，遍历顺序为：
\[
a \to e \to d \to f \to c \to b
\]

---

#### 4. 答案
选项中符合的遍历序列是：
\[
\textbf{D. aedfcb}
\]


### 题目

在图的邻接表中用**顺序存储**代替**头结点**的优点是什么？

---

### 解答

#### 1. 邻接表的结构
在图的存储中，邻接表是一种常用的结构，用于存储图的邻接关系：
- 每个顶点都有一个链表或数组存储与之直接相连的顶点。
- **传统邻接表**的实现通常在每个链表的开头使用一个“头结点”来存储起始点的信息，便于管理链表。

---

#### 2. 顺序存储代替头结点
若用**顺序存储**（如数组）代替每个链表的“头结点”，可以带来以下优点：

1. **节省存储空间**  
   - 使用头结点时，每个链表需要额外存储一个头结点，增加了存储开销。
   - 顺序存储直接将邻接点按序排列，避免了头结点的存储开销。

2. **访问效率更高**  
   - 顺序存储使得查找和访问某顶点的邻接点变得更加高效，因为邻接点存储在连续的内存区域，可以通过数组索引直接访问，无需遍历链表。

3. **实现简化**  
   - 使用顺序存储可以省去链表操作中涉及的动态内存分配和指针操作，简化了算法的实现和维护。

---

#### 3. 缺点
需要注意的是，顺序存储也有缺点，例如当邻接点数量动态变化时，顺序存储的灵活性不如链表，需要考虑数组扩容的问题。

---

### 总结
在图的邻接表中用**顺序存储**代替**头结点**的主要优点是：
\[
\textbf{节省存储空间，并且可以通过索引高效地访问每个顶点的邻接点。}
\]


### 题目解析

设散列表的长度为 \( 8 \)，散列函数 \( H(k) = k \mod 7 \)。初始记录关键字序列为 \( \{25, 31, 8, 27, 13, 68\} \)。要求分别计算以下两种冲突解决方法的**平均查找长度**：

1. **线性探测法**  
2. **链地址法**

---

### 解答

#### 散列基本信息
散列函数为 \( H(k) = k \mod 7 \)，表示关键字被映射到的散列表位置是：
\[
\text{位置索引} = k \mod 7
\]

---

### 1. 线性探测法
**步骤：**
- 当发生冲突时，检查下一个连续的存储位置，直到找到空位为止。

**关键字插入散列表：**
1. **25**: \( 25 \mod 7 = 4 \) → 插入位置 4。
2. **31**: \( 31 \mod 7 = 3 \) → 插入位置 3。
3. **8**: \( 8 \mod 7 = 1 \) → 插入位置 1。
4. **27**: \( 27 \mod 7 = 6 \) → 插入位置 6。
5. **13**: \( 13 \mod 7 = 6 \) → 冲突，线性探测，插入位置 7。
6. **68**: \( 68 \mod 7 = 5 \) → 插入位置 5。

最终散列表如下：
\[
\{ \_, 8, \_, 31, 25, 68, 27, 13 \}
\]

**查找分析：**
计算每个关键字的查找长度（插入时探测的次数）：
1. **25**: 直接插入，查找长度 = 1。
2. **31**: 直接插入，查找长度 = 1。
3. **8**: 直接插入，查找长度 = 1。
4. **27**: 直接插入，查找长度 = 1。
5. **13**: 冲突 1 次后插入，查找长度 = 2。
6. **68**: 直接插入，查找长度 = 1。

总查找长度：
\[
1 + 1 + 1 + 1 + 2 + 1 = 7
\]

平均查找长度：
\[
\text{平均查找长度} = \frac{7}{6} \approx 1.167
\]

---

### 2. 链地址法
**步骤：**
- 每个散列位置存储一个链表，冲突的关键字加入该链表。

**关键字插入散列表：**
1. **25**: \( 25 \mod 7 = 4 \) → 插入位置 4。
2. **31**: \( 31 \mod 7 = 3 \) → 插入位置 3。
3. **8**: \( 8 \mod 7 = 1 \) → 插入位置 1。
4. **27**: \( 27 \mod 7 = 6 \) → 插入位置 6。
5. **13**: \( 13 \mod 7 = 6 \) → 插入位置 6 的链表。
6. **68**: \( 68 \mod 7 = 5 \) → 插入位置 5。

最终散列表如下（链表表示）：
\[
\begin{aligned}
\text{位置 1}: & \ 8 \\
\text{位置 3}: & \ 31 \\
\text{位置 4}: & \ 25 \\
\text{位置 5}: & \ 68 \\
\text{位置 6}: & \ 27 \to 13 \\
\end{aligned}
\]

**查找分析：**
计算每个关键字的查找长度（链表中的位置）：
1. **25**: 位置 4，第 1 个元素，查找长度 = 1。
2. **31**: 位置 3，第 1 个元素，查找长度 = 1。
3. **8**: 位置 1，第 1 个元素，查找长度 = 1。
4. **27**: 位置 6，第 1 个元素，查找长度 = 1。
5. **13**: 位置 6，第 2 个元素，查找长度 = 2。
6. **68**: 位置 5，第 1 个元素，查找长度 = 1。

总查找长度：
\[
1 + 1 + 1 + 1 + 2 + 1 = 7
\]

平均查找长度：
\[
\text{平均查找长度} = \frac{7}{6} \approx 1.167
\]

---

### 最终结果
1. **线性探测法的平均查找长度**：1.167  
2. **链地址法的平均查找长度**：1.167
